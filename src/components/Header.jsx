  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-emerald-900/10 bg-[#FDFBF7]/70 backdrop-blur-md transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        {/* Brand Logo */}
        <div onClick={() => setActivePage('home')} className="flex items-center gap-2 group cursor-pointer">
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`relative transition-colors duration-200 group py-2 ${
                activePage === item.id ? 'text-orange-600' : 'text-emerald-800 hover:text-emerald-950'
              }`}
            >
              {item.label}
              <span className={`absolute bottom-0 left-0 h-[2px] bg-gradient-to-r from-emerald-600 to-orange-500 transition-all duration-300 ${
        ...
        {/* Mobile Toggle */}
        <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-emerald-800 hover:text-emerald-950">
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`block w-full text-left py-2 font-semibold ${
                activePage === item.id ? 'text-orange-600' : 'text-emerald-800'
              }`}
            >
              {item.label}
            </button>
