...
export default function Footer({ setActivePage }) {
  ...
  // Raw inline SVG templates ensuring zero compilation errors with your bundling environment
  const socialLinks = [
    { 
      name: "Instagram", 
      href: "https://instagram.com", 
      color: "hover:text-pink-600 hover:bg-pink-50 border-emerald-900/10 text-emerald-900/80 bg-emerald-50/40",
      svg: (
    ...
    { 
      ...
      color: "hover:text-blue-600 hover:bg-blue-50 border-emerald-900/10 text-emerald-900/80 bg-emerald-50/40",
      svg: (
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    { 
      ...
      color: "hover:text-sky-700 hover:bg-sky-50 border-emerald-900/10 text-emerald-900/80 bg-emerald-50/40",
      svg: (
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <footer className="w-full bg-[#FDFBF7] border-t border-emerald-950/10 pt-16 relative z-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-10 md:gap-8 pb-16">
        ...
        {/* Column 1: Core Institutional Message Statement */}
        <div className="md:col-span-5 space-y-4">
          ...
          <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed max-w-sm">
            Building inclusive social architectures by provisioning credit systems directly across deep rural sectors.
          </p>
        </div>

        {/* Column 2: State Tracking Navigation Menu Links */}
        <div className="md:col-span-3 space-y-4">
          ...
          <ul className="space-y-3">
            {['Home', 'About', 'Products', 'Contact'].map((p) => (
              <li key={p}>
                <button 
                  onClick={() => setActivePage(p.toLowerCase() === 'about' ? 'about' : p.toLowerCase() === 'contact' ? 'contact' : p.toLowerCase())} 
                  className="text-xs sm:text-sm text-slate-600 hover:text-emerald-800 font-bold tracking-tight inline-flex items-center gap-0.5 group transition-colors focus:outline-none"
                >
                  <span>{p === 'About' ? 'About Us' : p === 'Contact' ? 'Contact Us' : p}</span>
        ...
        {/* Column 3: Re-Engineered Regulatory Disclosures with UPPER RIGHT social icons */}
        <div className="md:col-span-4 space-y-4">
          <div className="flex items-center justify-between border-b border-emerald-900/5 pb-2">
            ...
            {/* 🌟 FIXED: Social platform array icons from image_281191.png integrated safely onto the upper right side 🌟 */}
      ...
      {/* LOWER FULL-BLEED RICH DARK BAR (Clean single-line configuration wrapper) */}
      <div className="w-full bg-[#02291a] border-t border-white/[0.06] py-8 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
          ...
          {/* Left Alignment Piece: Infrastructure Node Status Token */}
          ...
          {/* Right Alignment Piece: Crisp Uniform Single-line copyright block layout */}
          <div className="text-center md:text-right text-[11px] sm:text-xs text-emerald-100/60 font-semibold tracking-tight whitespace-nowrap">
            &copy; {currentYear} Kraaj Micro Finance Limited. All Rights Reserved. Licensed by the Reserve Bank of India.
          </div>
